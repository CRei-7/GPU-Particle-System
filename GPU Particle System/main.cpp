...
static const char* computeShader = "./particle.comp";
...
int main() {
	...
	ComputeShader.CreateComputeShader(computeShader);

	GLint deltaTimeLocation = glGetUniformLocation(ComputeShader.GetShaderID(), "deltaTime");
	GLint gravityLocation = glGetUniformLocation(ComputeShader.GetShaderID(), "gravity");
	...
	glBufferData(GL_SHADER_STORAGE_BUFFER, MAX_PARTICLES * sizeof(Particle), particlePool.particles.data(), GL_DYNAMIC_DRAW);// Syntax: (target, size, data, usage)

	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, particleSSBO);// Bind the buffer to the shader storage block binding point 0
	glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

	glBindBuffer(GL_ARRAY_BUFFER, particleSSBO);// Bind the buffer to the GL_ARRAY_BUFFER target for vertex attribute setup
	...
	while (!mainWindow.getShouldClose()) {
		...
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, particleSSBO);
		glBufferData(GL_SHADER_STORAGE_BUFFER, MAX_PARTICLES * sizeof(Particle), particlePool.particles.data(), GL_DYNAMIC_DRAW);
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);

		ComputeShader.UseShader();
		glUniform1f(deltaTimeLocation, deltaTime);
		glUniform1f(gravityLocation, gravity);
		glDispatchCompute((MAX_PARTICLES + 63) / 64, 1, 1); // Dispatch compute shader with enough work groups to cover all particles
		glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT | GL_VERTEX_ATTRIB_ARRAY_BARRIER_BIT); // Ensure that the compute shader has finished writing to the SSBO before rendering

		glBindBuffer(GL_SHADER_STORAGE_BUFFER, particleSSBO);
		glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, MAX_PARTICLES * sizeof(Particle), particlePool.particles.data());// Get the updated particle data from the SSBO back to the CPU side
		glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
